...
abruptness, source decomposition---are separately measured,
establishing abruptness as a phenotype rather than a smuggled
existence criterion. Second, we provide a calibrated measurement
...
and off-design physics. Finally, we show that in \emph{learned}
systems, collapse dissociates cleanly by timescale, varies
...
provenance boundary---that is frozen before adjudication. Objectivity
is ensured through this freezing, measured invariance, and refusal to
adjudicate outside the contract's domain (Methods).
...
instrument recovers the correct source in 72/72 cells. Crucially,
defending against the metric-artefact
...
Fig.~1b). A revelation-only or discontinuous-metric control produces
...
It is correctly absent in an ordinary supervised learner, whose collapse
...
This learned system cleanly separates two senses of emergence
...
by its own state machine, making it a positive control rather than a
discovery about learning. We therefore preregistered two systems in
...
contract-relativity. Together, these results provide a sharp statement:
higher-order structure is not unlearnable but \emph{unfavoured}.
...
0.199). Every subcritical run is correctly gated null, and the
...
the efficient plan, leaving no competing joint regimes. Restoring this
ingredient \emph{within the same standard package} restores
...
convention. A preregistered within-episode probe locates the
...
barrier collapses gradually, and our instrument correctly identifies
this.

% ==========================================================================
\section*{Discussion}

We show that emergence can be defined on the object that actually
reorganizes---the effective joint possibility space---and that this
...
provably separate axes, and a discontinuous scoring function produces
zero collapse. The definition is generative rather than nominal. It
supplies a source typology that transfers from analytic ground truth
...
Several boundaries
remain. First, the commitment window is not causally verified (a one-pulse
hypothesis failed its falsification clause; formation proved
re-entrant). Second, stored language-model checkpoints are too sparse for any
breakpoint claim. Third, representation-independence holds as a measured
equivalence class, breaking only where a representation erases the
latent channel. Regime-object audits sharpen this
boundary: mis-declared objects lose verdicts, never manufacture them (Methods). Finally, ``emergence $=$ onset breakpoint'' is not a universal
equivalence, as several canonical cases collapse gradually. What we offer
