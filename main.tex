...
labelled emergent or not depending on the measure. Here we
...
and higher-order channels. Calibrated on analytic ground truth and
...
and task success---through entirely different couplings (Fig.~1a),
...
ask how the effective possibility space closes; our instrument
attributes structure under a stated contract, not causal mechanism
...
possibility space has contracted onto building it. This
...
Fig.~1d)---contract-relativity made precise: ``higher-order'' is
...
37.7--45.5)---a strong partial replication: the strict thinning
clause passes in only 3/5, missing solely on thinned-subsample
detector power, and we do not adjust the threshold post hoc.
...
0.266 unaligned)---sharper than equilibrium-style width
...
The claim is falsifiable and its boundary reported honestly
...
closure can still be steered---complementary programmes, the collapse
object adding timing and control.
...
separates the two classes, on present evidence, is a joint
...
boundary we state rather than engineer away (a settled-envelope
extension failed its own validation and was retired;
Methods); certifying non-monotone commitment and extending to
large-scale self-play are the clear next steps. We do not claim a causally verified commitment
...
closes, and that earns its claims by reporting every place it says
no. A natural next step is to connect the collapse
