...
each field certifies it with a different instrument, so identical
macroscopic behaviours can be labelled emergent or not depending on
the measure chosen. Here we define emergence as a
spontaneous, persistent, regime-level collapse of a system's effective
joint state--action--trajectory possibility space, decomposed by
source into environment-mediated, individual, pairwise and
higher-order channels. Using a preregistered protocol, we calibrate
the instrument on analytic ground truth, validate it on off-design
physics, and apply it to learned multi-agent systems. An onset-type
breakpoint in the collapse appears where a system commits to
a new joint regime---including in learned populations with no designed
commitment gate---and not under ordinary smooth optimization; its
timing and sharpness shift systematically with system size and
distance from criticality, and remaining openness predicts whether an
intervention can still change the outcome. This provides a calibrated,
falsifiable operationalization of a contested word.
\end{abstract}
...
in advance\citep{anderson1972more,goldenfeld1999simple,bedau1997weak,holland1998emergence}.
The word \emph{emergence} names that phenomenon in statistical
physics\citep{stanley1971introduction,bak1987self}, in the
self-organization of animal
collectives\citep{camazine2001self,couzin2005effective,sumpter2006principles,ballerini2008interaction},
in the appearance of new capabilities in large neural
...
Yet the fields do not share an instrument. Integrated-information and causal-emergence measures ask
...
information-theoretic autonomy measures test for macro-variable
...
and the machine-learning literature reports ``emergent abilities'' as
sharp upturns of a benchmark against model
scale\citep{wei2022emergent,ganguli2022predictability,srivastava2022beyond},
a debate entangled with scaling
laws\citep{kaplan2020scaling,hoffmann2022training} and with grokking,
where generalization appears long after
memorization\citep{power2022grokking,nanda2023progress}.
...
sharp jump disappears\citep{schaeffer2023emergent}. This is not an
isolated embarrassment; it is the generic risk of defining emergence on
the \emph{observable of convenience}---a benchmark, an order parameter,
a success rate---rather than on the object that actually reorganizes.
Two systems can produce byte-identical macroscopic outcomes---identical
joint distributions, identical marginals, identical task
success---through entirely different underlying couplings (Fig.~1a),
and no statistic of that shared distribution can tell them apart. If
``emergence'' is read off the outcome, these systems are permanently
indistinguishable. They become distinguishable only relative to a
declared \emph{observation contract}---which variables are measured,
which are exogenous, and which interaction-severing counterfactuals
are admissible---under which one can ask how the effective possibility
space closes. That is what our instrument measures; it attributes
structure under a stated contract rather than claiming to recover
causal mechanism from outcome statistics alone.

We take the second view and make it operational. Consider the joint
space of what every part \emph{could} still do together---the effective
joint state--action--trajectory space. Before organization this space
is wide; a colony of ants approaching a gap can still form a bridge,
cross elsewhere, or disperse\citep{reid2015army}. Emergence, on our
account, is the moment this space \emph{collapses} onto a narrow
committed regime: the colony decides, together, to build. Crucially the
collapse is not the completion. The bridge does not yet exist when the
collective possibility space has already contracted onto building it.
This is the intuition our instrument is built to measure, and to
measure in a way whose verdicts are demonstrably stable within a
measured admissible equivalence class of contracts---reference space,
truncation, window, sampling grid and representation
(Methods)---rather than inheriting the fragility of the observable of
convenience.

Here we make three contributions. First, a definition with a two-level
structure: a qualification (spontaneous, regime-level, persistent) and a
quantitative \emph{emergence intensity profile} (EIP) whose
axes---amplitude, abruptness, and the source decomposition---are
separately measured, so that abruptness is a phenotype rather than a
smuggled existence criterion. Second, a calibrated measurement contract,
matured through preregistered self-falsification, that recovers the
source of a collapse on analytic ground truth (72/72 cells) and on
physics the instrument was never designed around. Third, and most
consequential for this journal's readers, the finding that in
\emph{learned} systems the same collapse admits a clean dissociation of
timescales, varies systematically and reproducibly with system size and
coupling, predicts controllability, and---importantly---is \textbf{not}
generic: the punctuated form arises spontaneously where learning must
cross a joint exploration barrier (convention formation, role lock-in),
while ordinary dense-reward multi-agent optimization collapses
smoothly, and we report exactly where it does and does not appear.
...
stochastic policy, we work throughout with the \emph{effective}
possibility count $N_\mathrm{eff}=2^{H}$; this is the formal version of
the $10^n \!\to\! 2^n$ intuition. A system \emph{qualifies} as emergent
...
persistent (the closed regime remains stable over the declared
observation horizon; recovery from perturbation and causal
irreversibility are reported separately as robustness properties, not
folded into the definition). Given qualification, we
report an intensity profile rather than a binary label. $P_t$ itself is
defined relative to a declared \emph{observation contract}---which
variables enter the joint space, over what horizon, at what
discretization, against which reference, with which provenance
...
domain, not through a claim of representation-free measurement
(Methods).
...
relational channels (all five preregistered checks pass; Fig.~1d). This
contract-relativity is not a bug: it is the precise, measurable
statement that ``higher-order'' is defined only relative to what an
observer declares exogenous. On a full factorial of 72 analytic cells
(4 sources $\times$ 3 temporal shapes $\times$ 2 stabilities $\times$ 3
magnitudes) the instrument recovers the source in 72/72 cells, and---the
point that defends us against the metric-artefact
critique\citep{schaeffer2023emergent}---the amplitude $M$ is invariant
across temporal shape (relative range 0.000 across all groups) while the
abruptness $J$ strictly orders punctuated $>$ sigmoid $>$ gradual
(Fig.~1c). Amplitude and abruptness are measured as different
quantities; a revelation-only or discontinuous-metric control produces
exactly zero collapse.

\subsection*{A breakpoint marks the punctuated phenotype, not emergence
itself}
...
gradual, non-monotone or unresolved collapse---absence of a breakpoint
therefore never by itself rules emergence out. We fit the
collapse curve with a two-regime model and compare it to a one-regime
fit by $\Delta$BIC, inside a saturation-truncated window and behind an
...
definitionally on fresh seeds rather than by tuning). The matured
detector was then frozen and scored on a held-out labelled benchmark:
zero false positives across deceleration, gradual and flat control
families at every tested grid density and noise level, onset power 1.00
at the operating resolution of our experiments, and location agreement
with a standard change-point method (Methods). Every headline verdict
below is additionally stable across 243 adjudication contracts spanning
gate, window, truncation, threshold and grid, and---separately, because
detector settings and measurement representations are different
things---across a preregistered battery of representations of the
openness object itself (state coarse-graining, behavioural sampling in
place of probabilities, object dualities, reference and truncation
changes), whose measured equivalence class, including the two
representations that break it and why, is reported in Methods.

With the matured detector, an \emph{onset}-type breakpoint (slow $\to$
fast) follows the preregistered profile predictions across the tested
systems---present where the theory predicts punctuated commitment,
absent where it predicts smooth convergence (Fig.~6c). It appears in a committing ant
colony, in a learned high-order coordination task (7/8 seeds pooled
across the original run and a preregistered five-seed extension), and
at the Kuramoto
synchronization transition\citep{kuramoto1984chemical,strogatz2000kuramoto}.
It is absent---correctly---in an ordinary supervised learner, whose
collapse begins at maximum rate and decelerates (a \emph{knee}, not an
onset), and it is unresolvable, and so not claimed, on stored
language-model checkpoints whose entropy has already collapsed before
the second saved step.

\subsection*{Abruptness scales with system size and criticality}

The breakpoint is not a detector artefact but a systematic feature of
...
rate---a derivation we recorded before running---the commitment time
should grow logarithmically while the collapse profile stays fixed.
...
0.266 unaligned)---a sharper statement than equilibrium-style width
rescaling\citep{stanley1971introduction}. One sub-prediction failed
and is reported as such: at the longer preregistered horizon the onset
threshold sits between $N=10$ and $N=20$, so ``onset at every
$N\geq10$'' was counted a miss rather than the bar being moved. In the Kuramoto system the
same instrument tracks the approach to criticality: across coupling $K=0.9\to2.5$ (10/10
runs onset) the breakpoint time falls monotonically (6.7 $\to$ 1.8,
critical slowing down approached from above) and the closing slope rises
monotonically (0.032 $\to$ 0.199), while every subcritical run is
correctly gated null and the collapse is carried by the pairwise channel
in 3/3 seeds (Fig.~6b)\citep{acebron2005kuramoto}. Abruptness therefore
has two independent, manipulable control parameters---system size and
...
delaying the choice of side. Trained with policy
gradients\citep{williams1992simple}, 10/10 seeds learn the task
(success $\geq 0.995$) and 10/10 exhibit the predicted signature: the
side-openness of the joint policy holds at a plateau for $\sim$17--19
steps and then collapses, with an onset breakpoint in every seed
($\Delta$BIC 45.8--52.7, $t^{*}$ concentrated at 16--18; Fig.~2b,d). A
...
structural delay, not learning per se, that produces punctuated collapse
(Fig.~2c). The phenomenon reproduces under a different
algorithm\citep{mnih2016asynchronous} (advantage actor--critic on the
byte-identical environment): 5/5 seeds reproduce the plateau-then-collapse
shape and the primary hinge ($\Delta$BIC 37.7--45.5, $t^{*}$ 14--16). We
report this as a strong partial replication rather than full algorithm
independence, because the strict robustness clause (surviving subsample
thinning in $\geq$4/5 seeds) passes in only 3/5---two seeds miss solely
on thinned-subsample detector power, and we do not adjust the threshold
post hoc.
...
(0 $\to$ $\sim$0.65 within $\sim$100 updates, 0/5 breakpoints), and at
fine 5-update resolution the success rise is fast but has no structural
onset (0/5). Capability formation is smooth and expansive; capability
realization is punctuated. This dissociation, observed in the same
system with the same instrument, clarifies why debates over whether
...
by its own state machine, so the flagship functions as a positive
control for the instrument rather than a discovery about learning. We
therefore preregistered two systems in which \emph{no} environment
mechanism delays commitment: every joint regime is available, and all
are exactly equivalent, from the first update. In a ten-agent
signalling population (five meanings, five symbols, reward only for
successful decoding\citep{lewis1969convention}), any of the 120
possible codes could be adopted at any moment; yet convention openness
...
700--1{,}025) in every onset seed, and the five seeds converge to five
\emph{different} codes. In a six-agent division-of-labour task (team
reward only when all six interchangeable roles are covered; chance
1.5\%), assignment openness shows the same signature in 5/5 seeds and
more strongly ($\Delta$BIC 53.6--71.7; closing slope $\sim$30$\times$
the plateau slope), again with collapse preceding capability in every
seed and five distinct role permutations across five seeds. In both
systems the open plateau \emph{is} the joint exploration barrier, and
we measure it rather than assert it: mid-plateau, a probe agent
unilaterally committing to the \emph{best} available code or role
gains essentially nothing (0.011; 0.001), whereas after commitment
the cheapest unilateral deviation forfeits the structural maximum of
the payoff (0.40 of a 0.40 ceiling; 1.00), and populations from
different seeds are mutually incompatible (cross-seed intelligibility
0.14, hybrid-team success 0.05, versus $\sim$1.00 within). A code or
a role division has value only to the extent that others already
share it---and the collapse is nucleated, self-amplifying
commitment, the learned analogue of the sharp consensus transitions
known from naming-game and convention-formation
studies\citep{baronchelli2006sharp,centola2015spontaneous}. Neither
system depends on its tabular parameterization: replacing the tables
with randomly initialized neural policies (per-agent MLPs for the
signalling population; one \emph{shared} MLP for all six role agents)
reproduces the signature in ten fresh seeds per system---onset in 6/7
learned convention seeds and 10/10 role seeds ($\Delta$BIC up to 162),
collapse preceding capability in every one, and every seed converging
to a different code or permutation. The neural runs also expose a
mechanism the theory predicts: random initialization pre-breaks the
symmetry among equivalent regimes, compressing the competition plateau
roughly tenfold (commitment within the first 5\% of training, which a
coarse checkpoint grid provably cannot resolve---the same power floor
measured in the detector benchmark), and an initialization-scale sweep
moves the commitment time in the predicted direction (monotone in the
signalling system; extremes correct but midpoint non-monotone in the
role system, reported as a registered miss). This mechanism---seed
randomness deciding when and where a run commits---gives a
regime-level reading of the finding that emergent language-model
capabilities reflect bimodal cross-seed performance distributions
rather than deterministic scale thresholds\citep{zhao2025random}:
...
possibility collapse is therefore not an artefact of designed gates or
of tabular parameterization: it arose reproducibly in every tested
system where learning must cross an endogenous joint-regime barrier,
and the barrier's width is set by how symmetric the competition
starts.
...
contract-relativity. Together these give a sharp statement: higher-order
structure is not unlearnable but \emph{unfavoured}---gradient learning
selects the lowest-order implementation available and builds irreducible
structure only when information constraints force
it\citep{tishby2000information}, echoing the difficulty of learning
compositional protocols in emergent-communication
studies\citep{mordatch2018emergence,lazaridou2017multi,foerster2016learning}
and value-factorization methods in cooperative
MARL\citep{lowe2017multi,sunehag2018value,rashid2018qmix}. Off design,
the same ladder correctly
reads out Kuramoto oscillators (uncoupled $\to$ null; common driver
$\to$ $C_\mathrm{env}$; single edge $\to$ $C_\mathrm{pair}$), a mechanism
...
through its environment-conditioned component ($C_\mathrm{env}$ 0.0137
[0.0123, 0.0156] vs 0.0005 [0.0004, 0.0007], non-overlapping; Fig.~4d),
even though a single-point coupling measure cannot---the
organizational difference is visible in the collapse composition,
under the fixed contract, though in no scalar summary.
...
0.0006 $\to$ 0.054 $\to$ 0.205 across openness bins; Fig.~5d); a closed
episode (openness $<$ 0.1) flips 0/2{,}600 times, and flipped episodes
are 0.58 openness units more open than unflipped ones (permutation
...
1.0 up to $t=16$ and only 0.27 by $t=30$, and side-openness predicts
switchability with AUC 0.996 (Fig.~5a,b). This is not a time proxy:
holding the intervention time fixed, per-episode openness still
separates flippable from locked episodes with AUC 0.974--0.990 at every
tested $\tau$ (20{,}480 episodes per cell). Conditioning additionally
on the physical order parameter removes the signal---the registered
boundary of the claim: in this system openness is an
information-sufficient transform of the physical state, so its value
is not extra predictive information but that it is computable from
the policy alone, without privileged access to the order parameter,
and transfers as a decision rule. Openness also
works \emph{prospectively}: an intervention rule
that simply waits until per-episode openness first drops to a threshold
calibrated on the original seeds transfers to five unseen seeds, where
it flips the outcome in 99.99\% of episodes while acting on average 3.8
steps later than the best fixed schedule (99.6\%) and beating random
timing by 21 points---remaining openness is a policy-accessible
intervention coordinate, not a retrospective correlate, and it acts
as late as each episode allows. A matched-parameter contrast
...
when the single stickiness parameter is set so no hidden phase
exists---all other constants imported unchanged---the advantage reverses
(0.811 vs 0.884; Fig.~5c). Openness carries controllability information
beyond the order parameter exactly when there is a hidden regime to
consolidate.

\subsection*{Where the punctuated form does not appear}

The claim is falsifiable and we report its boundary honestly (Fig.~6c).
Onset-type collapse is \textbf{not} generic to learned optimization. In
real deep multi-agent RL (Overcooked, two agents) the learned policies
show gradual trajectory-space collapse and increasing role selectivity
that track capability but produce no structural onset (0/3 on both a
policy-entropy and a trajectory-occupancy object). A registered rescue
sequence---smooth population consensus, quorum-threshold populations, and
learning-rate scans---also produced no onset (0/20 in the quorum
population).

This negative is not left unexplained: the theory names the missing
ingredient---cramped-room geometry pins the efficient plan, leaving no
equivalent competing joint regimes---and the ingredient can be restored
\emph{within the same standard package}. In the official
coordination-ring layout, clockwise and counterclockwise circulation
are mirror-equivalent joint conventions with value only when shared.
With identical PPO mechanics and budget, all eight preregistered
...
while neither cramped-room control ever commits on the same object at
the same checkpoint density. The frozen detector certifies a punctuated
onset in one seed, with the breakpoint (780k steps) preceding the
capability crossing (1M steps), and declines the rest: their convention
formation is non-monotone---commitment forms, dissolves and
re-forms---which our monotone-collapse instrument was not built to
certify. We count the registered onset-rate clause as a miss and report
it as an instrument boundary; the regime-level fact that survives every
seed is the committed final convention that cramped room never
develops. A preregistered within-episode probe locates the commitment
level precisely: at mid-training checkpoints (population still split
between directions), branched continuation rollouts show that episodes
drift with a direction bias but never internally lock (0/5 certified,
median within-episode collapse below the 0.1 gate), whereas at the
final checkpoints the same probe reads every episode as closed from
its first step (initial openness 0.37, one direction, 8/8 seeds) and
untrained controls stay open throughout with zero false onsets. The
ring convention is therefore a formation-level commitment with no
realization stage---the mechanistic complement of the grip system,
whose commitment lives entirely within the episode.

A final preregistered experiment tests whether this commitment is
causally real rather than a descriptive label. We added unbiased
Gaussian noise (two strengths) to stored checkpoints at three training
times---very early, at the population's most open point, and well
after commitment---and resumed training for 400k steps with
byte-identical mechanics (8 training seeds, 48 continuation runs; the
seed is the independent unit and all inference is at seed level). The
primary endpoint, declared in the preregistration, is whether the
perturbation \emph{disturbs} the commitment (flips the final
convention or decommits it): this happens in 7/8 seeds perturbed while
open versus 1/8 perturbed after commitment (exact paired sign-flip
test, $p=0.016$; at run level 8/16 versus 1/16, Fisher $p=0.008$).
Strict direction reversal is a secondary, descriptive endpoint: three
open-phase flips and zero late flips at run level, a contrast in the
predicted direction that does not reach significance across eight
seeds ($p=0.13$) and which we do not lean on. The stored openness at
the perturbed checkpoint predicts strict flips across the pooled 48
runs (AUC 0.85, preregistered threshold 0.70; descriptive, as runs
cluster within seeds; four of the five flips occurred at openness
$\geq 0.90$), and capability recovers to 0.92 of the unperturbed rate
after late perturbation---the same perturbation-plus-retraining budget
that can no longer move the convention is amply sufficient to relearn
the task. What locks at commitment is the institution, not the
learning. Two boundaries of this claim are explicit. The experiment
validates the functional distinction between \emph{open} and
\emph{committed} policy states in an unmodified community benchmark;
it does not certify a breakpoint time $t^{*}$ as a causal transition
point, because formation here is non-monotone and only one seed
carries a certified onset. And because open and late checkpoints
differ in training time as well as openness, the timing contrast alone
cannot exclude generic training maturity; the fixed-time analysis that
rules out the time confound exists in the grip system, not yet here. Among classic cases the picture is deliberately mixed:
Vicsek flocking\citep{vicsek1995novel}, Swift--Hohenberg pattern
selection\citep{cross1993pattern} and Schelling
segregation\citep{schelling1971dynamic} organize strongly but collapse
\emph{gradually} under our object, while the Potts
model\citep{wu1982potts} reproduces the known
continuous-versus-first-order distinction through hinge strength and
hysteresis. The honest conclusion is not ``everything is punctuated
collapse'' but a classification: possibility-collapse profiles sort
systems into punctuated, gradual, parameter-axis and early-warning
forms, and onset appears specifically when the system must cross a
joint-regime barrier---whether that barrier is physical (the grip
gate), informational (the blocked low-order channel) or endogenous to
learning itself (a convention or role structure that has value only
once shared). Dense-reward optimization with no such barrier collapses
gradually, and our instrument says so.

% ==========================================================================
\section*{Discussion}

We have argued, and measured, that emergence can be defined on the object
that actually reorganizes---the effective joint possibility space---rather
than on the observable of convenience, and that this choice avoids the
specific failure mode---discontinuous scoring of continuous
competence---behind the metric-artefact problem that has dogged
emergent-ability
claims\citep{schaeffer2023emergent}: amplitude and abruptness are provably
separate axes, and a discontinuous scoring function produces zero collapse
under our instrument. The definition is generative rather than nominal. It
supplies a source typology that transfers from analytic ground truth to
physics to learned agents; a breakpoint whose presence and absence track
the theory's own predictions across ant colonies, a synchronization
transition and learned high-order coordination; two systematic and
manipulable dependences of abruptness on system size and coupling; and a
control-theoretic payoff, since remaining openness
predicts whether an intervention can still flip the outcome.
...
are different events on different timescales: our learned flagship shows the first as smooth and
expansive and the second as punctuated, in the \emph{same} system. This
reframes the debate about whether capabilities ``suddenly'' appear: the
capability may form smoothly while each deployment of it commits abruptly.
Concurrent work detects higher-order structure in multi-agent
language-model groups through partial information decomposition of
time-delayed mutual information\citep{riedl2026emergent}; that
programme asks whether synergy is \emph{present}, whereas ours asks
when the joint possibility space \emph{closes}, how abruptly, through
which channel, and whether the closure can still be steered---the two
are complementary, and the collapse object adds the timing and control
dimensions that presence tests do not address.
...
controls, and we now know they are not load-bearing: convention
formation and role lock-in reproduce the punctuated signature with no
designed barrier at all. What separates the two classes, on present
evidence, is the presence of a joint exploration barrier, and the
coordination-ring experiment tests exactly this boundary inside the
standard benchmark that produced the negative: restoring equivalent
competing regimes restores committed conventions in every seed.
Certified onsets remain rare there because that system's commitment is
non-monotone---an instrument boundary we state rather than engineer
away (a preregistered settled-envelope extension failed its own
synthetic validation and was retired unused; Methods). Certifying
non-monotone commitment, and extending to large-scale self-play and
open-ended environments, are the clear next steps. We do not claim a causally verified commitment window---a
one-pulse-destroys-the-regime hypothesis failed its own falsification
clause, and formation proved re-entrant, which we report as a persistence
property. We make no breakpoint claim for stored language-model
checkpoints, whose grids are too sparse to resolve onset. We do not
claim the openness object is representation-independent in the
absolute: the audit delivers a measured equivalence class---verdict
and location survive object dualities, behavioural sampling and state
coarse-graining down to the detector's resolution floor, and break
when a representation erases the latent channel---not blanket
invariance. And
the universal equivalence ``emergence $=$ onset breakpoint'' remains
...
falsifiable instrument that measures \emph{how}, \emph{when} and
\emph{through which channel} a possibility space closes, and that earns
its claims by reporting, in the open, every place it says no. A natural
next step is to connect the collapse object to internal-representation
geometry in trained networks\citep{shai2024transformers}, extending the
instrument from behaviour to mechanism.
...
throughout). Zero observed false positives among 200 curves per control
...
curves. The zero power at 12 grid points is retained as an honest floor:
...
location. The collapse object itself is fixed by the theory (the joint
...
adjudication (\texttt{emergence\_certificate.py}) so that a new system
...
parameters. The hidden-coordination flagship uses 8 individually-observing
...
frozen detector. The neural replications keep both environments and
...
alongside the fine-grid result. The initialization sweep varies the
...
direction-reversal contrast to descriptive. The finite-size study uses sizes
...
recorded in place with its trigger. We characterize these as
...
prints every number cited in the text and figures, and by a
figure-generation script (\texttt{make\_figures.py}) that draws all
panels from the same files.
\normalsize
...
\small
Large-language-model agents were used throughout this work, under
continuous human direction and review, to design and implement
experiments, refine analysis protocols, write and debug code, and
draft and edit text. All predicted outcomes, falsification clauses and
adjudication thresholds were fixed in the protocol files before the
runs they govern; every number reported in the text and figures is
generated by the released code from raw experiment outputs and
cross-checked by an automated extraction script. The authors
independently reviewed and validated the code, protocols and analyses,
and take full responsibility for all claims.
\normalsize
...
\caption{\textbf{The problem and the instrument.}
\textbf{a},~The intuition: many parts wander in a wide joint possibility
space (grey) until a breakpoint $t^{*}$ after which trajectories commit to
one regime and abandon the alternatives (schematic).
\textbf{b},~Four generators---central script, common cause, coincidence and
local feedback---constructed so that no statistic of the shared
joint-action distribution separates them (identical joints, marginals
and macroscopic success). What separates them is the declared
contract: conditioning on declared exogenous variables,
interaction-severing counterfactuals and persistence checks assign
each to its predicted certificate component---attribution is
contract-relative, not mechanism discovery from outcome statistics.
\textbf{c},~Full-factorial calibration (72 cells): amplitude $M$ is
invariant across temporal shape while abruptness $J$ strictly orders
punctuated $>$ sigmoid $>$ gradual.
\textbf{d},~Source-decomposition ladder on analytic ground truth: each pure
...
grip before the object can be pushed, structurally delaying the choice of
side.
...
\textbf{b},~At fine 5-update resolution the success rise is fast but still
has no structural onset (0/5); dots mark per-seed midpoints.
\textbf{c},~Summary: capability formation is smooth and expansive;
capability realization is punctuated---same system, same instrument.}
\end{figure}
...
circles, sizes below the onset threshold. Inset: median collapse
curves for $N=50$--$500$ superpose under pure time translation
(aligned RMS 0.010 vs 0.266 unaligned)---the profile is universal,
