...
\title{\vspace{-1.5em}\bfseries Measuring collective emergence through
collapse of the effective joint possibility space}
...
labelled emergent or not. We define collective emergence as a spontaneous,
...
commitment. On the same data, amplitude thresholds,
information-theoretic composites and generic change-point detectors
each fail at least one of these adjudications. Possibility-space
collapse makes a contested concept
...
Here we introduce a two-level definition of collective emergence: a
qualification
...
abruptness is a phenotype, never an existence criterion. We then
...
irreversibility, are reported separately. The gates exclude by
construction: revealed structure fails endogeneity, contraction
carried by individual marginals alone is not regime-level, and
optimization toward a single uncontested optimum presents no
alternatives to collapse over (the dense-reward controls below never
commit to any regime). Collective learning is therefore not emergent
by default; it qualifies only when a population spontaneously and
persistently selects among competing joint regimes. The gates are
applied mechanically: a standardized certificate issues opposite
verdicts to a lone ant and an ant colony whose entropy contractions
are nearly identical, because the lone ant's collapse is carried
entirely by the individual source and the colony's by the joint
regime, so a bare entropy drop can never qualify on its own (Methods;
Supplementary Table~6). For qualified
systems, we report an intensity profile in place of a binary label. The
...
A preregistered baseline battery then gives the standard
information-theoretic toolkit its best a-priori shot on the same 72
cells: a composite rule built from marginal entropies, total
correlation and pairwise mutual information recovers 54/72, misreading
all 18 environment-driven cells as pairwise coupling, because a common
cause and genuine coupling are indistinguishable to any functional of
the system distribution without the contract's declared conditioning
step. A raw entropy-amplitude threshold fails in both directions at
once: it accepts the two external-takeover controls that the
instrument rejects as exogenous, and accepts none of the 72 true
positives (Methods; Supplementary Table~12).
...
(Methods). Generic change-point detectors do not reproduce this
operating point: calibrated to the same 5\% false-positive budget on
flat curves, binary segmentation and CUSUM both reach a power of 1.00
but fire on 100\% of deceleration controls (binary segmentation also
on 100\% of gradual controls; Supplementary Table~12). The two-regime
gate, not the change-point machinery, is what buys specificity. Every headline verdict below is additionally stable across
...
We next examine learned agents, whose
possibility space is itself a product of optimization.
...
In both systems the open plateau behaves as a joint exploration
barrier, measured here through its unilateral component (Methods).
...
episodes per cell). Because episodes nest within training seeds, the
race was re-aggregated at the seed level: side-openness reaches an AUC
of at least 0.999 within every individual seed, with a seed-cluster
bootstrap 95\% CI of 0.993--1.000 (Methods). Conditioning additionally on the physical order
...
episode allows. A matched-parameter contrast asks when openness adds
information beyond physical observables. In the pooled episode-level
race, openness ranks above the physical order parameter when stances
are inertial (a hidden consolidation phase exists; AUC 0.886 vs
0.849), and the ranking reverses when the single stickiness parameter
removes the hidden phase (0.811 vs 0.884); both pooled
orderings were preregistered predictions. A frozen seed-level addendum
(Fig.~5c) sustains the control-arm ordering in 5/5 seeds but not the
sticky-arm one: within every sticky seed the physical variable leads
(0.946--0.956 vs 0.910--0.931; 0/5, a registered miss). We therefore do not claim
that openness outperforms the order parameter in this system; both are
strong predictors, and the pooled sticky ordering is an aggregation
effect, not a per-seed superiority. The hidden phase's seed-level
effect is narrower: it closes most of the gap between openness and the
physical variable (0.025--0.036, versus 0.061--0.101 without it;
Methods).
...
$K=0.9\to2.5$ with ten seeds per coupling (50/50 onsets), the mean
breakpoint time falls strictly monotonically (6.64 $\to$ 1.80;
Spearman $\rho=-0.99$, permutation $p=10^{-5}$) and the mean closing
slope rises strictly monotonically (0.030 $\to$ 0.197; $\rho=0.98$,
$p=10^{-5}$), with per-coupling bootstrap intervals shown in Fig.~6b.
Every subcritical run is gated null, and the
...
the institution, not the learning. Because open
and late checkpoints also differ in training time, we ran a
preregistered fixed-time follow-up: all eight seeds perturbed at the
same step (960k, selected by a variance-maximizing rule frozen in
advance), where three seeds were still behaviourally open (openness
0.95--1.00) and five were committed. No run moved (0/16), a registered
miss of the openness-predicts-movability clause. Every continuation,
including the behaviourally open seeds, re-converged to its seed's
eventual direction, and the open seeds' lower task maturity does not
explain the null (Methods; Supplementary Table~13). Together the two
experiments bound the claim. The checkpoint-conditioned contrast
establishes a functional difference between empirically open and
committed populations, but the common-clock control did not reproduce
it, so openness is not isolated causally from the rest of training
history. What the null does show is that the eventual direction is
already parameter-committed while behaviour still looks open:
formation-level commitment runs deeper than behavioural openness, and
reading openness at a single fixed time is not a sufficient handle
for redirection.
...
Within a declared observation contract, we define collective
emergence on the
...
separates the two classes is a joint exploration barrier, whose
unilateral component---no single-agent gradient onto the regime
mid-plateau, a structural-maximum cost of leaving it after---is
measured directly (Methods). The
...
formation proved re-entrant. A registered trajectory-level estimator
of reachable futures showed the same re-entrance in a tractable system
and stopped at its validity gate, and a language-model convention
pilot stopped at a bias gate (Supplementary Note~1). Across every
closure notion we froze---behavioural, settled-envelope,
reachable-future---none is universally monotone: irreversibility
enters the framework as a measured property, never an assumption.
Stored language-model checkpoints are too
...
does not (Methods). With the analyst removed entirely, a frozen
discovery recipe recovers the grip system's two-regime structure in
every seed and its openness retains most of the fixed-time
controllability signal (AUC 0.73--0.93 versus 0.98--0.99 declared),
short of its registered bar: declaration buys measurement power, not
the phenomenon. Nor is ``emergence $=$ onset breakpoint'' a
...
with the nested ladder below, which avoids direct high-dimensional
estimation.
...
\paragraph{Method-baseline battery.}
A preregistered battery evaluates standard alternatives on exactly the
data the instrument used, with all rival definitions frozen before
implementation. On the 72-cell factorial, a composite of marginal
entropies, total correlation and pairwise mutual information---given
the fixed best-effort decision rule stated in the protocol---recovers
the source in 54/72 cells and misassigns all 18 environment-driven
cells as pairwise coupling; the composite has no environment rung
because, without the contract's declared conditioning step,
environmental and internal relational constraints are not separable by
any function of the system joint distribution. A raw
amplitude threshold ($C_\mathrm{peak}\geq 0.5$ of reference, no
qualification) accepts the two external-takeover pseudo-controls the
instrument rejects and accepts none of the 72 true positives. On the
matched-confound mechanisms, joint entropy, total correlation and
pairwise mutual information are equal across the three generators to
machine precision while the contract verdicts differ. On the held-out
curve benchmark, binary segmentation (RBF cost, gain acceptance) and
CUSUM, each calibrated to a 5\% false-positive rate on flat curves
before scoring any other family, reach power 1.00 but fire on 100\% of
deceleration curves (binary segmentation also on 100\% of gradual
curves); neither attains the instrument's operating point (power 1.00
at false-positive rate 0.000). One preregistration detail was wrong
and is retained: the protocol predicted the amplitude rule would
accept the revelation/metric controls, but those have zero entropy
amplitude and are rejected by the amplitude rule too; the clause
passed through the external-takeover controls instead (Supplementary
Table~12).
...
an individual-level object distinct from the joint convention;
...
and~5). A functional follow-up asks whether the machine-discovered
regime variable still carries the controllability information with
the analyst removed entirely: racing its openness against the
declared side-openness on the same 81{,}920 grip intervention
episodes (frozen protocol, appended preregistration), the discovered
predictor reaches fixed-time AUCs of
0.73--0.93 against 0.98--0.99 for the declared object, missing the
registered 0.80-everywhere bar (a registered miss), while raw policy
entropy carries almost none of the signal (pooled AUC 0.62 against
0.996). The discovery recipe recovers the two-regime structure in
5/5 seeds; what declaration buys is measurement power, not the
existence of the signal (Supplementary Table~14).

\paragraph{Statistical unit.}
Intervention episodes are nested within training seeds, so a frozen
analysis addendum re-aggregates the same race at the seed level, the
independent unit. Declared side-openness reaches a pooled AUC of at
least $0.999$ within every individual seed (seed-cluster bootstrap
95\% CI 0.993--1.000; leave-one-seed-out range 0.995--0.998),
discovered openness 0.73--0.85 (CI 0.730--0.851), and raw policy
entropy 0.60--0.64 (CI 0.606--0.636); side-openness beats entropy in
5/5 seeds (both registered outcomes met; Supplementary Table~15).
The cross-seed pooled values quoted above are lower than every
within-seed value because seed-to-seed offsets in the predictors add
between-cluster variance; the ordering of predictors is identical
under both aggregations. A parallel addendum re-aggregates the stance
races (Fig.~5c) at the seed level: the control-arm ordering (physical
variable above openness) holds in 5/5 seeds, but the pooled sticky-arm
ordering does not (0/5; physical 0.946--0.956 against openness
0.910--0.931 within every seed, seed-cluster bootstrap 95\% CIs
0.893--0.947 against 0.916--0.928), so the sticky-arm superiority is a
registered miss and is withdrawn in the main text (Supplementary
Table~16).
...
because its collapse is carried entirely by the individual source,
with no joint regime involved,
and the Overcooked occupancy object falls below the collapse
...
way. A planned second-layout study on the official
\texttt{counter\_circuit} layout, whose central circuit supports the
same two mirror-equivalent circulation conventions, was stopped at
the same gate: in three disclosed one-seed pilots (4M steps with the
standard shaping anneal; 8M with a lengthened anneal; 6M with shaping
never annealed) the fixed training recipe delivers zero soups, so no
confirmatory protocol was frozen. In the annealed pilots a
shaped-reward circulation habit forms and then dissolves once shaping
expires---a convention without realized task value, which is
precisely what the competence precondition exists to exclude. The causal commitment test perturbs stored ring checkpoints at
...
checkpoints differ in training time as well as openness, a
preregistered fixed-time follow-up perturbed all eight seeds at the
same step, 960k, chosen by a frozen rule (the common-grid checkpoint
maximizing cross-seed openness variance with $\geq 20$ committed
episodes per seed), with identical noise scales, resume length and
scoring. No continuation moved (0/16; seed-level Fisher $p=1.0$), a
registered miss of the openness-predicts-movability clause at fixed
time. The open seeds' lower checkpoint task performance (0.27--0.53
versus 0.67--1.9 mean soups) rules out maturity as an explanation of
the null, and every continuation re-converged to its seed's eventual
direction. The finite-size study uses sizes
...
intervention times.
\textbf{c},~Matched-parameter contrast at the seed level: each dot is
one training seed's AUC (five seeds per arm). Within every sticky seed
the physical variables lead openness (0/5), so the pooled sticky-arm
advantage of openness is an aggregation effect and is not claimed
(registered miss; Methods); the control-arm ordering holds in every
seed.
...
rises as coupling increases; points are means of ten seeds per
coupling, error bars are 95\% bootstrap intervals over seeds, and
subcritical runs are gated null.
