...
labelled emergent or not depending on the measure chosen. Here we
...
space, decomposed by source into environmental, individual, pairwise
and higher-order channels. Calibrating the instrument on analytic
ground truth and validating it on off-design physics, we find that
learning populations commit abruptly wherever a joint exploration
barrier---a convention or role with value only once shared---must be
crossed, and smoothly otherwise; that a capability can form smoothly
...
These instruments disagree in a way that is not cosmetic. An
influential critique showed that some reported emergent abilities are
metric artefacts: change the score function and the sharp jump
disappears\citep{schaeffer2023emergent}. This is the generic risk of
defining emergence on the \emph{observable of convenience}---a
benchmark, an order parameter, a success rate---rather than on the
object that actually reorganizes.
...
and no statistic of that shared distribution can tell them apart. They
become distinguishable only relative to a declared \emph{observation
contract}---which variables are measured, which are exogenous, and
which interaction-severing counterfactuals are admissible---under
which one can ask how the effective possibility space closes. That is
what our instrument measures; it attributes structure under a stated
contract rather than claiming to recover causal mechanism from outcome
statistics alone.
...
committed regime---the colony decides, together, to build---and the
collapse is not the completion: the bridge does not yet exist when the
possibility space has already contracted onto building it. Our
instrument measures this with verdicts demonstrably stable within a
measured admissible equivalence class of contracts (Methods), rather
than inheriting the fragility of the observable of convenience.
...
measured, so abruptness is a phenotype rather than a smuggled
existence criterion. Second, a calibrated measurement contract,
...
physics it was never designed around. Third, the finding that in
\emph{learned} systems the collapse dissociates cleanly by timescale,
varies systematically with system size and coupling, predicts
...
and we report exactly where it does and does not appear.
...
horizon; recovery from perturbation and causal irreversibility are
reported separately as robustness properties). Given qualification, we
...
relational channels (all five preregistered checks pass;
Fig.~1d)---contract-relativity is the precise, measurable statement
that ``higher-order'' is defined only relative to what an observer
...
strictly orders punctuated $>$ sigmoid $>$ gradual (Fig.~1c); a
...
fixed definitionally on fresh seeds rather than by tuning. The matured
...
battery of representations of the openness object itself, whose
measured equivalence class, including the representations that break
it and why, is reported in Methods.
...
absent where it predicts smooth convergence. It appears in a committing ant
...
training\citep{williams1992simple} yields 10/10 learned seeds
(success $\geq 0.995$) and 10/10 with the predicted signature:
...
plateau-then-collapse shape and primary hinge ($\Delta$BIC 37.7--45.5).
We report a strong partial replication: the strict thinning clause
passes in only 3/5, missing solely on thinned-subsample detector
power, and we do not adjust the threshold post hoc.
...
Formation is smooth and expansive; realization is punctuated. Observed
in the same system with the same instrument, this dissociation
...
measured rather than asserted: mid-plateau, a probe agent
...
for all six role agents) reproduce the signature in ten fresh seeds
per system---onset in 6/7 learned convention seeds and 10/10 role
seeds ($\Delta$BIC up to 162), collapse preceding capability in every
one, every seed converging to a different code or permutation. The
neural runs expose a mechanism the theory predicts: random
initialization pre-breaks the symmetry among equivalent regimes,
compressing the competition plateau roughly tenfold (Methods), and an
...
value-factorization\citep{lowe2017multi,sunehag2018value,rashid2018qmix}
...
though a single-point coupling measure cannot---the organizational
difference is visible in the collapse composition though in no scalar
summary.
...
intervention coordinate that acts as late as each episode allows. A matched-parameter contrast
...
(0.811 vs 0.884; Fig.~5c). Both orderings were the registered
predictions of their respective runs. Openness carries controllability
...
and is reported as such: at the longer horizon the onset threshold
sits between $N=10$ and $20$, so ``onset at every $N\geq10$'' was
counted a miss. In the Kuramoto system the same instrument tracks the
...
The claim is falsifiable and we report its boundary honestly
(Fig.~6c). Onset-type collapse is \textbf{not} generic to learned
optimization: in standard two-agent Overcooked the learned policies
collapse gradually on both a policy-entropy and a trajectory-occupancy
object (0/3), and a registered rescue sequence---smooth consensus,
quorum-threshold populations, learning-rate scans---also produced no
onset (0/20 in the quorum population).
...
direction bias but never internally lock (0/5 certified), whereas
final-checkpoint episodes are closed from their first step (initial
openness 0.37, 8/8 seeds; untrained controls stay open with zero false
...
stored checkpoints very early, at the population's most open point,
and well after commitment, then resumed training for 400k steps with
byte-identical mechanics (8 training seeds, 48 continuation runs; the
seed is the independent unit). The preregistered primary
...
predicts strict flips across the pooled runs (AUC 0.85; descriptive,
as runs cluster within seeds), and capability recovers to 0.92 of the
unperturbed rate after late perturbation: what locks at commitment is
...
(formation is non-monotone; open and late checkpoints also differ in
training time; Methods).

Among classic cases the picture is deliberately mixed: Vicsek
...
and hysteresis. The honest conclusion is a classification, not
``everything is punctuated'': collapse profiles sort systems into
...
function produces zero collapse under our instrument. The definition
...
breakpoint whose presence and absence track the theory's own
predictions, systematic and manipulable dependences of abruptness on
system size and coupling, and a control-theoretic payoff, since
remaining openness predicts whether an intervention can still flip
the outcome.
...
closure can still be steered---complementary programmes, with the
collapse object adding the timing and control dimensions.
...
controls, and they are not load-bearing: convention formation and role
lock-in reproduce the punctuated signature with no designed barrier at
all. What separates the two classes, on present evidence, is a joint
exploration barrier---now directly measured---and the coordination-ring
experiment tests exactly this boundary inside the standard benchmark
that produced the negative: restoring equivalent competing regimes
...
boundary we state rather than engineer away (a preregistered
settled-envelope extension failed its own synthetic validation and was
retired unused; Methods); certifying non-monotone commitment, and
extending to large-scale self-play and open-ended environments, are
the clear next steps. We do not claim a causally verified commitment
window (a one-pulse hypothesis failed its falsification clause;
formation proved re-entrant). We make no breakpoint claim for stored
language-model checkpoints, whose grids are too sparse. We do not
claim absolute representation-independence: the audit delivers a
measured equivalence class, breaking only where a representation
erases the latent channel. And the universal equivalence ``emergence
$=$ onset breakpoint'' remains unproven: several canonical cases
collapse gradually. What we offer is narrower and, we believe, more
useful---a calibrated, falsifiable instrument that measures \emph{how},
\emph{when} and \emph{through which channel} a possibility space
closes, and that earns its claims by reporting, in the open, every
place it says no. A natural next step is to connect the collapse
