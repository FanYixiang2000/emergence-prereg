...
The policy is left-right equivariant: at the symmetric state left/right
logits are tied, while nonzero object position/velocity can be amplified.
This tests whether a learned policy can keep initial episode openness and
collapse within the episode without specifying a side externally.
"""
...
def rollout_batch(policy: EquivariantPolicy, batch: int, train: bool = True):
    x = torch.zeros(batch)
    v = torch.zeros(batch)
...
def eval_policy(policy: EquivariantPolicy, batch: int = 4096):
    with torch.no_grad():
...
def main() -> None:
    ...
    report = {
        "status": "LEARN-TRANSPORT-EQUIVARIANT learned realization quadrant; preregistered",
    ...
    out = OUTPUTS / "learn_transport_equivariant.json"
