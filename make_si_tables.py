...
# Supplementary Table 16: seed-level statistics for the stance races
ssu = load("learn_stance_stat_unit.json")["registered_outcomes"]
rows = []
for arm in ("sticky", "control"):
    per = ssu["per_seed_auc"][arm]
    ci = ssu["boot_ci_95_seed_cluster"][arm]
    loo = ssu["leave_one_seed_out_open"][arm]
    for key, label in [("open", "Joint openness"),
                       ("absx", "Physical $|x|$")]:
        vals = [p[key] for p in per]
        rows.append([arm if key == "open" else "",
                     label, f"{min(vals):.3f}--{max(vals):.3f}",
                     f"[{ci[key][0]:.3f}, {ci[key][1]:.3f}]",
                     f"{min(loo):.3f}--{max(loo):.3f}"
                     if key == "open" else "---"])
table(
    r"\textbf{Seed-level statistics for the stance races (registered "
    r"miss).} The sticky and matched-control stance races of Fig.~5c "
    r"re-aggregated at the seed level (deterministic reruns of the "
    r"published arms; 16{,}384 episodes per seed, 5 seeds per arm). "
    r"SSU1 (openness above $|x|$ in $\geq$4/5 sticky seeds) failed at "
    r"0/5; SSU2 (physical variable above openness in $\geq$4/5 control "
    r"seeds) passed at 5/5. The pooled sticky-arm ordering reported in "
    r"Fig.~5c is therefore an aggregation effect and the corresponding "
    r"superiority claim is withdrawn. CI: seed-cluster bootstrap "
    r"(10{,}000 resamples). LOSO: leave-one-seed-out pooled AUC of "
    r"openness.",
    ["Arm", "Predictor", "Per-seed AUC", "Cluster bootstrap 95\\% CI",
     "LOSO AUC"],
    rows, "si:tab:stancestat",
    align="p{0.09\\textwidth}p{0.2\\textwidth}p{0.17\\textwidth}"
    "p{0.22\\textwidth}p{0.15\\textwidth}")
