...
def _demo() -> None:
    """Render intervention traces for manual audit (see
    INDEPENDENT_AUDIT_INSTRUCTIONS.md step 4)."""
    import json as _json
    policy = TeamPolicy("scripted_roles", cook_agent=0)
    for layout in ("cramped_room", "asymmetric_advantages"):
        print(f"=== {layout} ===")
        for mode in (None, "do_commit", "do_block"):
            for k in range(3):
                row = run_episode(policy, layout, 4200 + k, mode)
                print(f"  {mode or 'natural':10s} seed {4200 + k}: "
                      f"{_json.dumps(row)}")
...
if __name__ == "__main__":
    ...
    _ap.add_argument("--demo-episode", action="store_true")
    if _ap.parse_args().demo_episode:
        _demo()
